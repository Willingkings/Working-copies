...
scenicAreaControls.controller("travelScenicAreaList",function ($sce,$timeout,$q, $scope,$httpService,$state, $stateParams) {
    ...
    // 获取跳转参数缓存
    $scope.areaSelect = $scope.areaSelect == null ? JSON.parse(sessionStorage.getItem('scenicSelect')) : $stateParams.scenicSelect; // 业务参数+缓存
...
scenicAreaControls.controller("scenicAreaDetails",function ($q, $rootScope, $scope, $httpService, $state, $stateParams) {
    /************************************************** 图片懒加载 ****************************************************/
    $scope.lozy = function () {
        var timeM = 0;  // 定义秒数为0
        var time = setInterval(function(){
...
scenicAreaControls.controller("scenicAreaTicket",function ($q, $rootScope,$scope, $http,$httpService,$state, $stateParams, $sce, $uibModal, $location, Md5) {
    /************************************************** 图片懒加载 ****************************************************/
    $scope.lozy = function () {
        var timeM = 0;  // 定义秒数为0
        var time = setInterval(function(){
    ...
    $scope.goSpecialPay = function (obj){
        if ($rootScope.isApp) {
            ...
            if (isLogin) {
                ...
                obj.getSign = JSON.parse($scope.getSign);
                obj.getUserInfo = JSON.parse($scope.getUserInfo);
